...
- `docs/index.mdx` — landing page rendered at `/docs`.
- Additional Markdown/MDX files are grouped in the sidebar using the `"navigation.tabs"` configuration inside `mint.json`.
...
- Confirm each slug in `"navigation.tabs"` points to a Markdown/MDX file inside `docs/` (paths omit the `.mdx` extension).
